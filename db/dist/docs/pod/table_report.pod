
=head1 NAME

report

=head1 DESCRIPTION



=head1 FIELDS

 identifier (character varying) - A unique identifier for the report.
 title (character varying) - 
 url (character varying) - 
 organization (character varying) - 
 doi (character varying) - 
 _public (boolean) - 


=head1 SCHEMA

                                    Table "gcis_metadata.report"
     Column    |       Type        |  Modifiers   | Storage  |             Description             
 --------------+-------------------+--------------+----------+-------------------------------------
  identifier   | character varying | not null     | extended | A unique identifier for the report.
  title        | character varying |              | extended | 
  url          | character varying |              | extended | 
  organization | character varying |              | extended | 
  doi          | character varying |              | extended | 
  _public      | boolean           | default true | plain    | 
 Indexes:
     "report_pkey" PRIMARY KEY, btree (identifier)
 Foreign-key constraints:
     "report_organization_fkey" FOREIGN KEY (organization) REFERENCES organization(identifier)
 Referenced by:
     TABLE "_report_editor" CONSTRAINT "_report_editor_report_fkey" FOREIGN KEY (report) REFERENCES report(identifier)
     TABLE "_report_viewer" CONSTRAINT "_report_viewer_report_fkey" FOREIGN KEY (report) REFERENCES report(identifier)
     TABLE "chapter" CONSTRAINT "chapter_ibfk_1" FOREIGN KEY (report) REFERENCES report(identifier) MATCH FULL
     TABLE "figure" CONSTRAINT "figure_report_fkey" FOREIGN KEY (report) REFERENCES report(identifier)
     TABLE "finding" CONSTRAINT "finding_report_fkey" FOREIGN KEY (report) REFERENCES report(identifier)
 Triggers:
     audit_trigger_row AFTER INSERT OR DELETE OR UPDATE ON report FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func('true')
     audit_trigger_stm AFTER TRUNCATE ON report FOR EACH STATEMENT EXECUTE PROCEDURE audit.if_modified_func('true')
     delpub BEFORE DELETE ON report FOR EACH ROW EXECUTE PROCEDURE delete_publication()
     updatepub BEFORE UPDATE ON report FOR EACH ROW WHEN (new.identifier::text <> old.identifier::text) EXECUTE PROCEDURE update_publication()
 Has OIDs: no
 

=cut


