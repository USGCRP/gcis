...
=head1 SCHEMA
 ...
 -------------------------+-------------------+--------------+----------+-------------------------------------
  ...
  organization_identifier | character varying |              | extended | 
  doi                     | character varying |              | extended | 
  _public                 | boolean           | default true | plain    | 
 Indexes:
     "report_pkey" PRIMARY KEY, btree (identifier)
 Foreign-key constraints:
     "report_organization_fkey" FOREIGN KEY (organization_identifier) REFERENCES organization(identifier) ON UPDATE CASCADE
 Referenced by:
     ...
     TABLE "figure" CONSTRAINT "figure_report_fkey" FOREIGN KEY (report_identifier) REFERENCES report(identifier)
     TABLE "finding" CONSTRAINT "finding_report_fkey" FOREIGN KEY (report_identifier) REFERENCES report(identifier)
 Triggers:
